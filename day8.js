...
function parseData(indata) {
    ...
    return indata.trim().replace(/ \| /g, " ").split("\n").reduce((p, v) => {
...
function part1(input) {
    ...
    data.forEach(function (row) {
        ...
        if (retNumValue(d1).num) {
            total++
        }
        if (retNumValue(d2).num) {
            total++
        }
        if (retNumValue(d3).num) {
            total++
        }
        if (retNumValue(d4).num) {
            total++
        }
...
function getNumberData(realNumber, num) {
    let ret = false;

    realNumber.forEach(function (n) {
        if(n.num === num) {
            ret = n;
        }   
    });
...
function fillMissingNumberWithHint(realNumber, findHint, lengthHint, numberToFind) {
...
function fillMissingNumbers(realNumber) {

    let num4data = getNumberData(realNumber, 4);
    let num1data = getNumberData(realNumber, 1);

    realNumber = fillMissingNumberWithHint(realNumber, num4data.digit, 6, 9);
    realNumber = fillMissingNumberWithHint(realNumber, num1data.digit, 6, 0);
    realNumber = fillMissingNumberWithHint(realNumber, false, 6, 6);

    realNumber = fillMissingNumberWithHint(realNumber, num1data.digit, 5, 3);

    let num9data = getNumberData(realNumber, 9);
    let num3data = getNumberData(realNumber, 3);

    let num5Hint = arrayMinus(num9data.digit, num3data.digit);

    realNumber = fillMissingNumberWithHint(realNumber, num5Hint, 5, 5);
    realNumber = fillMissingNumberWithHint(realNumber, false, 5, 2);
...
function part2(input) {
    ...
    data.forEach(function (row) {
        ...
        row.forEach(function (n, index) {
            realNumber[index] = retNumValue(n);
        });

        realNumber = fillMissingNumbers(realNumber);

        let panelNumber = 
            (realNumber[10].num * 1000) + 
            (realNumber[11].num * 100) +
            (realNumber[12].num * 10) +
            (realNumber[13].num * 1);
